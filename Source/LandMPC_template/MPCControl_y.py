...
class MPCControl_y(MPCControl_base):
    ...
    def _setup_controller(self) -> None:
        ...
        f_combined=np.vstack([f,m]).flatten()
        ...
        s_vector_2x1 = cp.vstack([s] * F.shape[0]) 
        s_matrix_2xN = cp.hstack([s_vector_2x1] * self.N)

        constraints.append(F @ x_var[:, :-1] <= f.reshape(-1, 1)+ s_matrix_2xN)

        # Input constraints
        constraints.append(M @ u_var <= m.reshape(-1, 1))

        # Terminal Constraints -> dropped
